...
export const initVisualData = (initDataStr = '') => {
  console.log('initVisualData', initDataStr);
  let localData: any = {};
  try {
    localData = JSON.parse(initDataStr || (sessionStorage.getItem(localKey) as string));
  } catch (err) {
    localData = {};
  }
